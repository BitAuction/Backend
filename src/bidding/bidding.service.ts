...
export class BiddingService {
  ...
  async bid(org: string, userId: string, auctionID: string, price: number) {
    ...
    try {
      ...
      // Simulation version
      await this.simulationService.submitBid(auctionID, userId, org, price);
      const bids = await this.simulationService.getAllBids(auctionID);
      return { bids };
    } catch (error) {
      // gateway.disconnect();
      throw error;
  ...
  async getHighestBid(org: string, userId: string, auctionID: string) {
    ...
    try {
      ...
      // Simulation version
      const highestBid = await this.simulationService.getHighestBid(auctionID);
      return highestBid;
    } catch (error) {
      // gateway.disconnect();
      throw error;
